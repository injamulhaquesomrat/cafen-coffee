      ...
      {shopTabItems &&
        ...
        shopTabItems.map((item) => (
          <button
            ...
            className={`inline-flex w-full sm:w-fit justify-center border border-primary px-[30px] sm:px-10 py-2.5 rounded-full bg-secondary-foreground text-primary cursor-pointer ${
              selectedCategory === item.name.toLowerCase() && "bg-secondary"
            } hover:bg-secondary transition duration-300 ease-in-out`}
          >
            {item.name}
          </button>
        ))}
