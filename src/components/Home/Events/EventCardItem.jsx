      ...
      <div className="flex flex-col md:flex-row border border-cf-coffee rounded-2xl text-cf-coffee overflow-hidden lg:col-span-1">
        ...
        <div className="flex flex-col p-4 md:p-6 md:w-1/2">
          <div className="flex-1 pb-4 md:pb-16">
            <p>{item.date}</p>
            <h4 className="text-xl md:text-2xl font-sober pb-2 md:pb-4 tracking-tight">
              {item.title}
            </h4>
          ...
          <div>
            <Link to={`/event/${item.id}`}>
              <button className="px-6 py-1.5 bg-cf-yellow border border-cf-coffee rounded-full font-sober inline-flex items-center gap-2 group">
                View event{" "}
